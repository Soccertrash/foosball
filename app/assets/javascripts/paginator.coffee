###
Usage: 
paginator = new Paginator(<urlToConfig>, <baseUrl>)
paginator.setup() 

The urlToConfig must deliver a PaginationConfiguration object (JSON)
The baseUrl must provide a method <baseUrl>/list which actually loads the data
###
class window.Paginator
